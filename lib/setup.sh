...
g++ -shared -o libHPGeTreeEvent.so`root-config --ldflags`  -std=c++11 -fPIC -O3 -I`root-config --incdir` HPGeTreeEventDict.cxx
g++ -shared -o libHPGeSegTreeEvent.so`root-config --ldflags`  -std=c++11 -fPIC -O3 -I`root-config --incdir` HPGeSegTreeEventDict.cxx
